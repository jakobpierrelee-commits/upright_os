...
#if defined(ARDUINO_ARCH_ESP32) && !defined(LED_BUILTIN)
#define LED_BUILTIN 2
#endif

#ifndef UPRIGHT_RUNTIME_VERSION
#define UPRIGHT_RUNTIME_VERSION "balance_mvp_v2_core_esp32.1.0"
#endif
...
namespace Pins {
  ...
  const uint8_t ENC_LEFT = 32;
  const uint8_t ENC_RIGHT = 33;
  const uint8_t PWMA = 25;
  const uint8_t PWMB = 26;
  const uint8_t AIN1 = 27;
  const uint8_t STBY = 13;
  const uint8_t BIN1 = 14;
  const uint8_t VOL = 34;
#else
...
#if FEAT_EEPROM_CONFIG && defined(ARDUINO_ARCH_ESP32)
  static bool ready = false;
  if (!ready) {
    ready = EEPROM.begin(512);
  }
  return ready;
#else
  return true;
...
void setup() {
  Serial.begin(115200);
  delay(120);
  Serial.println(F("UPRIGHT_BALANCE_MVP_V2_CORE_ESP32_BOOT"));
