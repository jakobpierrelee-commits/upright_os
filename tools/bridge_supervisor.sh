...
pid_is_bridge() {
  ...
  ps -p "${pid}" -o args= 2>/dev/null | grep -q "${ROOT_DIR}/app/bridge/server.py --supervised --instance ${BRIDGE_INSTANCE}"
}
...
start_bridge() {
  ...
  (
    cd "${ROOT_DIR}"
    nohup python3 -u "${ROOT_DIR}/app/bridge/server.py" --supervised --instance "${BRIDGE_INSTANCE}" >"${LOG_FILE}" 2>&1 &
    echo $! >"${PID_FILE}"
  )
  local new_pid
  new_pid="$(bridge_pid)"
