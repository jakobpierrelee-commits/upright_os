/*
  compile_upload_preflight_test_v1
  Simple firmware contract to validate compile, upload, and preflight plumbing.

  - Implements the minimum STATUS telemetry keys expected by the UpRight bridge.
  - Responds to GET, PID, MOTION, SETPOINT, LIMITS, ARM/DISARM, ESTOP, CAL ZERO, and SAVECFG.
  - Emits deterministic synthetic telemetry so UI graphs move even without hardware.
*/
...
static void emitStatus() {
  ...
  Serial.print(g_mode == MODE_ARMED ? F("BALANCING") : F("SAFE_IDLE"));
  Serial.print(F(" estop="));
  Serial.print(g_estop ? F("1") : F("0"));
  Serial.print(F(" ang="));
  ...
  Serial.print(g_outer_loop_enabled ? F("1") : F("0"));
...
static void updateSyntheticTelemetry(float dt_s) {
  if (dt_s <= 0.0f) return;
  g_phase += dt_s * 0.8f;
  const float motion_wave = sinf(g_phase);
  g_raw = g_accel_level_offset + motion_wave * 1.2f;
  g_gyro = cosf(g_phase) * 12.5f;
  const float trim = g_upright_trim;
  g_vel_meas = motion_wave * 0.15f;
  g_vel_target = g_outer_loop_enabled ? 0.05f : 0.0f;
  g_target_angle_from_velocity = g_vel_target * g_kv;
  g_ang = (g_raw - trim) * 0.9f + trim;
  ...
  g_pid_u_sat = clampf(g_pid_u_unsat, -g_limits.out_max, g_limits.out_max);
  g_output_saturated = fabsf(g_pid_u_unsat - g_pid_u_sat) > 0.0001f;
  g_out = g_pid_u_sat;
  g_prev_err = err;

  const bool led_on = (g_mode == MODE_ARMED) && !g_estop;
  digitalWrite(PIN_LED, led_on ? HIGH : LOW);
  if (PIN_GATE_ENABLE >= 0) {
    digitalWrite(PIN_GATE_ENABLE, led_on ? HIGH : LOW);
  }
}
...
static void processCommand(char* line) {
  ...
  if (equalsToken(token, "MOTION")) {
    ...
    g_kv = atof(kv);
    g_kx = atof(kx);
    g_outer_loop_enabled = fabsf(g_kv) > 0.0001f;
    sendOk(F("MOTION"));
    return;
  }
...
void setup() {
  ...
  Serial.println(F("# compile_upload_preflight_test_v1 ready"));
}
