...
static void latchFault(uint16_t code, const __FlashStringHelper* note) {
...
static void printHelp() {
  Serial.println(F("OK HELP GET ARM DISARM ESTOP PID SETPOINT LIMITS FILTER KAL CAL ZERO SAVECFG LOADCFG DEFAULTCFG FAULTCLR LOGT LOGCSV BURSTCSV CSVHDR IDENT MOTOR_TEST"));
...
static void runMotorTest(const char* line) {
  ...
  const bool moved = labs(primary_delta) >= 1;
...
static void handleCommandLine(char* line) {
  ...
  if (strcmp(line, "IDENT") == 0) { Serial.println(F("UPRIGHT_PROFILED_RUNTIME_V1_1")); return; }
...
static void controlTick() {
  ...
  if (!g_imu_ok) {
    latchFault(FAULT_SENSOR_INVALID, F("imu_not_ready"));
  } else {
    g_mpu.update();
    g_raw = readAccelDeg();
    ...
    if (!isFinitef(g_raw) || !isFinitef(g_state.gyro_dps) || fabs(g_state.gyro_dps) > 2200.0f || fabs(g_raw) > 180.0f) {
      latchFault(FAULT_SENSOR_INVALID, F("sensor_invalid"));
    }
    ...
    if (!isFinitef(g_state.angle_deg)) {
      latchFault(FAULT_SENSOR_INVALID, F("angle_invalid"));
    }
  }
  ...
  if (g_state.armed && !g_state.estop_latched && g_state.active_fault == FAULT_NONE) {
    if (fabs(g_state.angle_deg) > g_cfg.tip_deg) {
      latchFault(FAULT_MODULE_UNHEALTHY, F("tip_limit"));
    } else {
      g_pid_err = g_cfg.set_deg - g_state.angle_deg;
      ...
      if (fabs(g_out) > 70.0f && (millis() - g_last_moving_enc_ms) > 500U) {
        latchFault(FAULT_ENCODER_STALE, F("encoder_stale"));
      }
    }
  } else {
    g_out = 0.0f;
  ...
  if (g_last_step_us > MAX_CONTROL_STEP_US) {
    ...
    if (g_loop_overrun_consecutive >= 5U) {
      latchFault(FAULT_LOOP_OVERRUN, F("loop_overrun"));
    }
  } else {
    g_loop_overrun_consecutive = 0;
  }
...
void setup() {
  ...
  if (imu_status == 0) {
    ...
    g_imu_ok = false;
    latchFault(FAULT_SENSOR_INVALID, F("imu_begin_failed"));
  }
